...
func (sq *StockQuery) QueryTrades() *TradeDateQuery {
	query := (&TradeDateClient{config: sq.config}).Query()
...
func (sq *StockQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (sq *StockQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Stock, error) {
	...
	if query := sq.withTrades; query != nil {
		if err := sq.loadTrades(ctx, query, nodes,
			func(n *Stock) { n.Edges.Trades = []*TradeDate{} },
			func(n *Stock, e *TradeDate) { n.Edges.Trades = append(n.Edges.Trades, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withSubscribers; query != nil {
		if err := sq.loadSubscribers(ctx, query, nodes,
			func(n *Stock) { n.Edges.Subscribers = []*User{} },
			func(n *Stock, e *User) { n.Edges.Subscribers = append(n.Edges.Subscribers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StockQuery) loadTrades(ctx context.Context, query *TradeDateQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *TradeDate)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tradedate.FieldStockID)
	}
	query.Where(predicate.TradeDate(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(stock.TradesColumn), fks...))
	}))
...
func (sq *StockQuery) loadSubscribers(ctx context.Context, query *UserQuery, nodes []*Stock, init func(*Stock), assign func(*Stock, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Stock)
	nids := make(map[uint64]map[*Stock]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(stock.SubscribersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(stock.SubscribersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(stock.SubscribersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(stock.SubscribersPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Stock]struct{}{byID[outValue]: {}}
...
func (sq *StockQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(stock.Table, stock.Columns, sqlgraph.NewFieldSpec(stock.FieldID, field.TypeUint64))
	_spec.From = sq.sql
