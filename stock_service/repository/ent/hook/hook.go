...
// The TradeDateFunc type is an adapter to allow the use of ordinary
// function as TradeDate mutator.
type TradeDateFunc func(context.Context, *ent.TradeDateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TradeDateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TradeDateMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TradeDateMutation", m)
}
