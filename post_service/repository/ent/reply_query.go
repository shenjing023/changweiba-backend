...
func (rq *ReplyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Reply, error) {
	...
	if query := rq.withOwner; query != nil {
		if err := rq.loadOwner(ctx, query, nodes, nil,
			func(n *Reply, e *Comment) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withParent; query != nil {
		if err := rq.loadParent(ctx, query, nodes, nil,
			func(n *Reply, e *Reply) { n.Edges.Parent = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withChildren; query != nil {
		if err := rq.loadChildren(ctx, query, nodes,
			func(n *Reply) { n.Edges.Children = []*Reply{} },
			func(n *Reply, e *Reply) { n.Edges.Children = append(n.Edges.Children, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReplyQuery) loadChildren(ctx context.Context, query *ReplyQuery, nodes []*Reply, init func(*Reply), assign func(*Reply, *Reply)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(reply.FieldParentID)
	}
	query.Where(predicate.Reply(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(reply.ChildrenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
