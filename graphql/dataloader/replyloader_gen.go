...
// NewReplyLoader creates a new ReplyLoader given a fetch, wait, and maxBatch
func NewReplyLoader(config ReplyLoaderConfig) *ReplyLoader {
	return &ReplyLoader{
		fetch:    config.Fetch,
		wait:     config.Wait,
...
func (l *ReplyLoader) Clear(key int) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}
...
func (b *replyLoaderBatch) keyIndex(l *ReplyLoader, key int) int {
...
func (b *replyLoaderBatch) startTimer(l *ReplyLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
